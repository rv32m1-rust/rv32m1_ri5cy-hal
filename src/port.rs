...
pub mod $portx {
    ...
    // Visit through pointer; generating pac::PORTX from MaybeUninit is probably uninitialized
    // thus cause error that is very hard to debug
    ...
    impl PortExt for pac::$PORTX {
        ...
        fn split(self, pcc_port: &mut pcc::$PORTX) -> Result<Self::Parts, EnableError> {
            ...
            Ok(Parts {
                $(
                    $ptxi: $PTXi { _function: PhantomData },
                )+
            })
        }
    }
...
$(
    ...
    impl<AF> $PTXi<AF> {
        port_impl!(@af0 $PTXi, $pcri, $af0);
        port_impl!(@afi $PTXi, "1", into_af1, ALT1, $pcri, mux_1, $af1);
        port_impl!(@afi $PTXi, "2", into_af2, ALT2, $pcri, mux_2, $af2);
        port_impl!(@afi $PTXi, "3", into_af3, ALT3, $pcri, mux_3, $af3);
        port_impl!(@afi $PTXi, "4", into_af4, ALT4, $pcri, mux_4, $af4);
        port_impl!(@afi $PTXi, "5", into_af5, ALT5, $pcri, mux_5, $af5);
        port_impl!(@afi $PTXi, "6", into_af6, ALT6, $pcri, mux_6, $af6);
        port_impl!(@afi $PTXi, "7", into_af7, ALT7, $pcri, mux_7, $af7);
    }
    ...
    (@af0 $PTXi: ident, $pcri: ident, $doc: expr) => {
    ...
    (@afi $PTXi: ident, $afi: expr, $into_afi: ident, $ALTi: ty, $pcri: ident, $mux_i: ident, x) => {}; // generate nothing
    (@afi $PTXi: ident, $afi: expr, $into_afi: ident, $ALTi: ty, $pcri: ident, $mux_i: ident, $doc: expr) => {
